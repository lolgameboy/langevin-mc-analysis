...
void plotConvergenceNd(int dim, double cov_factor, double stepsize, int pixelSideSize, double referenceN, double beta, double delta, double diminAdapt1, double diminAdapt2, bool plotPrecond, bool skipLowNCounts) {
	// Create gaussian
	...
	matplot::ylabel("RMSE");
	std::string name = std::format("Convergence {}D-LMC, covf={}, stepsize={}, pixels={}x{}, beta={}, adaptationCoeff={}", funct.dim, cov_factor, stepsize, pixelSideSize, pixelSideSize, beta, diminAdapt1);
	matplot::title(name);
	matplot::legend();
